...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      include 'comm.var'
      include 'mpif.h'
      integer i, j, k, t, fanal, t0, t1, i_ini, tt_final
...
c            call secondary_pert(t,1.d0)
     ...
     &                      + dt6 * ( dv1z(i,j,k) + dv2z(i,j,k) )
            ...
            if (maxlocal_wz < 1d-6 .and. t .ge. stpp) then 
              write(*,*) 'Stationary state.' 
              call escreve(t)      
              return
            end if
c           if (t .ge. tt) then
c             if (mod(t,stpp/64).eq.0)   call escreve_secondary(t)
c             if (mod(t-2,stpp/64).eq.0) call escreve_secondary(t)
c             if (mod(t-4,stpp/64).eq.0) call escreve_secondary(t)
c             if (mod(t,stpp/16).eq.0)   call escreve_iso(t)
c           endif  
...
c             call ts3d_pert(t, 1.d0)
     ...
     &                      + dt6 * ( dv1t(i,j,k) + dv2t(i,j,k) )
            ...
            if (mod(t,stpp) .eq. 0) call escreve(t)
            maxlocal_wz = maxval( abs( wz1(:,:,2) - wz(:,:,2) ) )
            maxlocal_th = maxval( abs( th1(:,:,2) - th(:,:,2) ) )
            call isstat(maxlocal_wz)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &                 c(ptsx,jmax,kfour),      d(ptsx,jmax,kfour)
      ...
      ! linear or non-linear product calculations
      call lterms_th(a,b,c,d,uth,vth,wth)
c     call nlterms_th(a,b,c,d,uth,vth,wth)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do i = i1 + 1, i2 - 1
        do k = 2, 2
          uy(i,1,k)    = dcmplx(fct*fcx2(i), 0.d0)
          d2uydx2(i,k) = dcmplx(fct*fcllx2(i), 0.d0)
        end do
      end do
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &       fcx2(imax), fcllx2(imax), ep, A     
      ...
      do i = 1, ptsx 
         uy(i,1,2)    = dcmplx(A*fcx2(i+shift), 0.d0)
         d2uydx2(i,2) = dcmplx(A*fcllx2(i+shift), 0.d0)
      end do

c     H = 1.d0
c     fct = 0.d0
c     do f = 1, 16
c       fct = 1.5d-2*dsin(H*(dble(t)+tempt)*omega*dt*dble(f))+fct
c     enddo

c     do i = 1, ptsx
c       uy(i,1,2)    = uy(i,1,2)    + dcmplx(0.d0,fcx(i+shift)*fct)
c       d2uydx2(i,2) = d2uydx2(i,2) + dcmplx(0.d0,fcllx(i+shift)*fct)
c     end do
